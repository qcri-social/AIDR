 ...
 *
 */
public class OutputErrorHandler {
	
	private static Logger logger = Logger.getLogger(OutputErrorHandler.class);
	
	public static void sendErrorMail(String code, String errorMsg) {
		Response clientResponse = null;
		Client client = ClientBuilder.newBuilder().register(JacksonFeature.class).build();
		try {
			WebTarget webResource = client.target(OutputConfigurator.getInstance().getProperty(OutputConfigurationProperty.TAGGER_REST_URI)
					+ "/misc/sendErrorEmail");
			
			Form form = new Form();
			form.param("module", "AIDROutput");
