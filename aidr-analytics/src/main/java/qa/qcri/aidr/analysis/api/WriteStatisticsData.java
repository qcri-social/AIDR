...
import qa.qcri.aidr.analysis.utils.AnalyticsConfigurationProperty;
...
public class WriteStatisticsData extends ChannelBufferManager implements
	...
	public void contextInitialized(ServletContextEvent sce) {
		...
		System.out
				.println("[contextInitialized] Initializing channel buffer manager");
		...
		System.out
				.println("[contextInitialized] Done initializing channel buffer manager");
	...
	public Response ping() {
		JsonObject obj = Json.createObjectBuilder()
				.add("aidr-analysis/writeData", "RUNNING").build();
	...
	private ReturnCode writeOutputDataToTagDataDB(Long granularity,
			Long timestamp) {
		// System.out.println("tagDataMap size: " + (tagDataMap != null ?
		...
		try {
			for (String key : tagDataMap.keySet()) {
				...
				// data[2] = labelCode
				TagDataMapRecord tCount = (TagDataMapRecord) tagDataMap
						.get(key);
				TagData t = new TagData(data[0], timestamp, granularity,
						data[1], data[2], tCount.getCount(granularity));
				System.out.println("Will attempt to persist data for: "
						+ t.getCrisisCode() + ", " + t.getAttributeCode()
						+ ", " + t.getLabelCode() + ", " + t.getTimestamp()
						+ ", " + t.getGranularity() + ": " + t.getCount());
				tagDataEJB.writeData(t);
				tCount.resetCount(granularity);
				TagDataMapRecord temp = (TagDataMapRecord) tagDataMap.get(key);
				// System.out.println("[writeOutputDataToTagDataDB] After reset, count for key = "
				// + key + " is = " + temp.getCount(granularity));

			}
		} catch (Exception e) {
			System.err
					.println("[writeOutputDataToTagDataDB] Error in writing to TagDataDB table!");
			e.printStackTrace();
	...
	private ReturnCode writeOutputDataToConfDataDB(Long granularity,
			Long timestamp) {
		try {
			for (String key : confDataMap.keySet()) {
				...
				// data[2] = labelCode
				// data[3] = bin
				ConfDataMapRecord fCount = (ConfDataMapRecord) confDataMap
						.get(key);
				ConfidenceData f = new ConfidenceData(data[0], timestamp,
						granularity, data[1], data[2],
						Integer.parseInt(data[3]), fCount.getCount(granularity));
				confDataEJB.writeData(f);
				fCount.resetCount(granularity);
				ConfDataMapRecord temp = (ConfDataMapRecord) confDataMap
						.get(key);
				// System.out.println("[writeOutputDataToConfDataDB] After reset, count for key = "
				// + key + " is = " + temp.getCount(granularity));
			}
		} catch (Exception e) {
			System.err
					.println("[writeOutputDataToConfDataDB] Error in writing to ConfidenceDataDB table!");
			e.printStackTrace();
			return ReturnCode.ERROR;
		}
		return ReturnCode.SUCCESS;
	 ...
	 */
	@Override
	public void manageChannelBuffersWrapper(final String subscriptionPattern,
			final String channelName, final String receivedMessage) {
		// System.out.println("Firing manageChannelBuffer on message from channel: "
		// + channelName);
		manageChannelBuffers(subscriptionPattern, channelName, receivedMessage);
		if (null == channelName) {
			logger.error("Something terribly wrong! Fatal error in: "
					+ channelName);
		} else {
			try {
				...
				if (classifiedTweet != null
						&& classifiedTweet.getNominalLabels() != null) {
					channelMap.putIfAbsent(classifiedTweet.getCrisisCode(),
					...
					for (NominalLabel nb : classifiedTweet.getNominalLabels()) {
						...
						if (tagDataMap.containsKey(tagDataKey)) {
							TagDataMapRecord t = (TagDataMapRecord) tagDataMap
									.get(tagDataKey);
							...
							tagDataMap.put(tagDataKey, t);
							System.out
									.println("[manageChannelBuffersWrapper] New Tag map entry with key: "
											+ tagDataKey
											+ " value = "
											+ tagDataMap.get(tagDataKey));
						}
						if (confDataMap.containsKey(confDataKey)) {
							ConfDataMapRecord f = (ConfDataMapRecord) confDataMap
									.get(confDataKey);
							f.incrementAllCounts();
							confDataMap.put(confDataKey, f);
						} else {
							ConfDataMapRecord t = new ConfDataMapRecord(
									granularityList);
							confDataMap.put(confDataKey, t);
							System.out
									.println("[manageChannelBuffersWrapper] New Conf map entry with key: "
											+ confDataKey
											+ " value = "
											+ confDataMap.get(confDataKey));
						}
					}
				}
			} catch (Exception e) {
		...
		lastTagDataCheckedTime = periodicInactiveChannelCheck(
				lastTagDataCheckedTime, tagDataMap);
		lastConfDataCheckedTime = periodicInactiveChannelCheck(
				lastConfDataCheckedTime, confDataMap);
	}

	private long periodicInactiveChannelCheck(long lastCheckedTime,
		...
		if (currentTime - lastCheckedTime > CHECK_INTERVAL) {
			for (String key : channelMap.keySet()) {
				if ((currentTime - channelMap.get(key)) > NO_DATA_TIMEOUT) {
					System.out
							.println("[periodicInactiveChannelCheck] Deleting data for inactive channel = "
									+ key);
					int deleteCount = deleteMapRecords(key, dataMap);
					System.out
							.println("[periodicInactiveChannelCheck] Deleted records count for inactive channel <"
									+ key + "> is = " + deleteCount);
				}
			}
		}
	...
	private int deleteMapRecords(final String key,
			ConcurrentHashMap<String, Object> dataMap) {
		int count = 0;
		for (String keyVal : dataMap.keySet()) {
			MapRecord data = (MapRecord) dataMap.get(key);
			synchronized (dataMap) {
				if (keyVal.contains(key) && data != null
						&& data.isCountZeroForAllGranularity()) {
			...
			while (runFlag) {
				...
				if (granularityList != null) {
					for (Long granularity : granularityList) {
						if (0 == lastTagDataWriteTime.get(granularity)
								|| (currentTime - lastTagDataWriteTime
										.get(granularity)) >= granularity) {
							// Write to DB table
							ReturnCode retVal = writeOutputDataToTagDataDB(
									granularity, currentTime);
							lastTagDataWriteTime.put(granularity, currentTime);
							System.out.println("[ToTagDataDB] retVal = "
									+ retVal);
							if (ReturnCode.SUCCESS.equals(retVal)) {
								System.out
										.println("[ToTagDataDB] Successfully wrote for granularity: "
												+ granularity
												+ " at time = "
												+ new Date(currentTime));
							}
						}
						if (0 == lastConfDataWriteTime.get(granularity)
								|| (currentTime - lastConfDataWriteTime
										.get(granularity)) >= granularity) {
							// Write to DB table
							ReturnCode retVal = writeOutputDataToConfDataDB(
									granularity, currentTime);
							System.out.println("[ToConfDataDB] retVal = "
									+ retVal);
							lastConfDataWriteTime.put(granularity, currentTime);
							if (ReturnCode.SUCCESS.equals(retVal)) {
								System.out
										.println("[ToConfDataDB] Successfully wrote for granularity: "
												+ granularity
												+ " at time = "
												+ new Date(currentTime));
							}
					...
					try {
						Thread.sleep(granularityList.get(0) - 1000); // sleep
																		// for
																		// the
																		// minimum
																		// granularity
